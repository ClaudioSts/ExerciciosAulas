function computePawnMoves(board, position) {
    ...
    const oneForward = addDirectionToPosition(position, {line: 1, column: 0}, multiplier)
    if (isEmpty(board, oneForward)) {
        moves.push(oneForward)

        const twoForward = addDirectionToPosition(position, {line: 2, column: 0}, multiplier)
        if (isFirstMove(board, position) && isEmpty(board, twoForward)) {
            moves.push(twoForward)
        }
    }

    const diagonalRight = addDirectionToPosition(position, {line: 1, column: 1}, multiplier)
    if (isEnemy(board, diagonalRight, player)) {
        moves.push(diagonalRight)
    }
    const diagonalLeft = addDirectionToPosition(position, {line: 1, column: -1}, multiplier)
